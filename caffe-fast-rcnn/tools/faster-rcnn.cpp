...
void draw_boxes(Mat& image, vector<pair<string, vector<float> > >& boxes, float time) {
	for (int i = 0; i < boxes.size(); i++) {
		...
		putText(image, text, Point(round(boxes[i].second[0]), round(boxes[i].second[1] + 15)), 2, 0.7, cv::Scalar(0, 0, 0), 3);
		putText(image, text, Point(round(boxes[i].second[0]), round(boxes[i].second[1] + 15)), 2, 0.7, cv::Scalar(255, 255, 255), 1);
		if (time > 0) {
			sprintf(text, "%.3f sec", time);
			putText(image, text, Point(10, 10), 2, 0.5, cv::Scalar(0, 0, 0), 3);
