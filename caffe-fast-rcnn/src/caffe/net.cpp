...
void Net<Dtype>::GetOutput(vector<pair<string, vector<Dtype> > >& boxes) {
	...
	for (int i = 1; i < cls_prob.shape(1); i++) {
		if (net_param().class_enable(i - 1) == 0) continue;
...
#ifdef PROF_LAYERS
  memset(prof_tick, 0, sizeof(prof_tick));
  prof_count = 0;
#endif
}
...
#ifdef PROF_LAYERS
  prof_tick[start][0] = clock();
#endif
  for (int i = start; i <= end; ++i) {
    // LOG(ERROR) << "Forwarding " << layer_names_[i];
    Dtype layer_loss = layers_[i]->Forward(bottom_vecs_[i], top_vecs_[i]);
#ifdef PROF_LAYERS
    prof_tick[i + 1][0] = clock();
    prof_tick[i][1] += prof_tick[i + 1][0] - prof_tick[i][0];
#endif
    loss += layer_loss;
    if (debug_info_) { ForwardDebugInfo(i); }
  }
#ifdef PROF_LAYERS
  prof_tick[end + 1][1] += prof_tick[end + 1][0] - prof_tick[start][0];
  prof_count++;
  if ((prof_count % 100) == 0) {
    printf("----------------------------------------------------------------\n");
    for (int i = start; i <= end; ++i) {
      printf("%s, %.2f%%\n", layer_names_[i].c_str(), prof_tick[i][1] * 100.0f / prof_tick[end + 1][1]);
    }
	printf("----------------------------------------------------------------\n");
  }
#endif
